import { useState } from 'react';
import { PixelCard } from '../ui/PixelCard';
import { PixelButton } from '../ui/PixelButton';
import { StatusPill } from '../ui/StatusPill';
import { ScanQrModal } from '../modals/ScanQrModal';
import { Camera, FileText, Shield, CheckCircle2, XCircle, ExternalLink } from 'lucide-react';

export function ScanVerifyScreen() {
  const [inputMode, setInputMode] = useState<'paste' | 'scan'>('paste');
  const [vcInput, setVcInput] = useState('');
  const [verificationResult, setVerificationResult] = useState<any>(null);
  const [isScanning, setIsScanning] = useState(false);

  const handleVerify = () => {
    // Mock verification
    setVerificationResult({
      isValid: true,
      vcId: 'vc_001',
      issuer: 'UIDAI',
      holder: 'did:pixel:0x742d...f0bEb',
      issuedDate: '2025-01-15',
      onChainStatus: 'confirmed',
      blockNumber: 12345678,
      txHash: '0xabc123...def456',
      reasons: [
        'Signature verified ✓',
        'Not revoked ✓',
        'Within validity period ✓',
        'On-chain record found ✓',
      ],
    });
  };

  return (
    <div className="p-4 space-y-4 bg-[#0f0f0f]">
      {/* Mode Toggle */}
      <div className="grid grid-cols-2 gap-2">
        <button
          onClick={() => setInputMode('paste')}
          className={`p-4 border-4 pixel-corners transition-all ${
            inputMode === 'paste'
              ? 'bg-[#1a1a1a] border-[#4F46E5]'
              : 'bg-[#0a0a0a] border-[#2a2a2a] hover:border-[#3a3a3a]'
          }`}
        >
          <FileText 
            className={`w-6 h-6 mx-auto mb-2 ${
              inputMode === 'paste' ? 'text-[#4F46E5]' : 'text-[#888]'
            }`}
            strokeWidth={3}
          />
          <div className={`pixel-text text-[10px] ${
            inputMode === 'paste' ? 'text-white' : 'text-[#888]'
          }`}>
            PASTE JSON
          </div>
        </button>

        <button
          onClick={() => {
            setInputMode('scan');
            setIsScanning(true);
          }}
          className={`p-4 border-4 pixel-corners transition-all ${
            inputMode === 'scan'
              ? 'bg-[#1a1a1a] border-[#06B6D4]'
              : 'bg-[#0a0a0a] border-[#2a2a2a] hover:border-[#3a3a3a]'
          }`}
        >
          <Camera 
            className={`w-6 h-6 mx-auto mb-2 ${
              inputMode === 'scan' ? 'text-[#06B6D4]' : 'text-[#888]'
            }`}
            strokeWidth={3}
          />
          <div className={`pixel-text text-[10px] ${
            inputMode === 'scan' ? 'text-white' : 'text-[#888]'
          }`}>
            SCAN QR
          </div>
        </button>
      </div>

      {/* Input Area */}
      {inputMode === 'paste' && (
        <PixelCard>
          <div className="space-y-3">
            <div className="pixel-text text-xs text-[#888]">PASTE VC JSON OR HASH</div>
            <textarea
              value={vcInput}
              onChange={(e) => setVcInput(e.target.value)}
              placeholder='{"vcId": "vc_001", "claims": {...}}'
              className="w-full h-32 bg-[#1a1a1a] border-4 border-[#2a2a2a] pixel-corners p-3 pixel-text text-[10px] text-white placeholder:text-[#555] focus:border-[#4F46E5] outline-none resize-none"
            />
            <PixelButton variant="primary" fullWidth onClick={handleVerify}>
              <Shield className="w-4 h-4 inline mr-2" />
              VERIFY
            </PixelButton>
          </div>
        </PixelCard>
      )}

      {/* Verification Result */}
      {verificationResult && (
        <PixelCard>
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <div className="pixel-text text-sm text-white">VERIFICATION RESULT</div>
              {verificationResult.isValid ? (
                <CheckCircle2 className="w-6 h-6 text-[#10B981]" strokeWidth={3} />
              ) : (
                <XCircle className="w-6 h-6 text-[#EF4444]" strokeWidth={3} />
              )}
            </div>

            <div className={`p-4 border-4 pixel-corners ${
              verificationResult.isValid 
                ? 'bg-[#10B981]/10 border-[#10B981]' 
                : 'bg-[#EF4444]/10 border-[#EF4444]'
            }`}>
              <div className={`pixel-text text-xs ${
                verificationResult.isValid ? 'text-[#10B981]' : 'text-[#EF4444]'
              }`}>
                {verificationResult.isValid ? 'VALID CREDENTIAL' : 'INVALID CREDENTIAL'}
              </div>
            </div>

            {/* Details */}
            <div className="space-y-2">
              <div className="flex items-center justify-between py-2 border-b-2 border-[#1a1a1a]">
                <span className="pixel-text text-[10px] text-[#888]">VC ID</span>
                <span className="pixel-text text-[10px] text-white">{verificationResult.vcId}</span>
              </div>
              <div className="flex items-center justify-between py-2 border-b-2 border-[#1a1a1a]">
                <span className="pixel-text text-[10px] text-[#888]">Issuer</span>
                <span className="pixel-text text-[10px] text-white">{verificationResult.issuer}</span>
              </div>
              <div className="flex items-center justify-between py-2 border-b-2 border-[#1a1a1a]">
                <span className="pixel-text text-[10px] text-[#888]">Issued</span>
                <span className="pixel-text text-[10px] text-white">{verificationResult.issuedDate}</span>
              </div>
              <div className="flex items-center justify-between py-2">
                <span className="pixel-text text-[10px] text-[#888]">On-chain</span>
                <StatusPill status="success" label="CONFIRMED" />
              </div>
            </div>

            {/* Reasons */}
            <div className="space-y-2">
              <div className="pixel-text text-[10px] text-[#888]">VERIFICATION CHECKS</div>
              {verificationResult.reasons.map((reason: string, idx: number) => (
                <div key={idx} className="flex items-center gap-2">
                  <div className="w-2 h-2 bg-[#10B981]"></div>
                  <span className="pixel-text text-[10px] text-white">{reason}</span>
                </div>
              ))}
            </div>

            {/* Transaction Link */}
            <button className="w-full flex items-center justify-between p-3 bg-[#1a1a1a] border-4 border-[#2a2a2a] pixel-corners hover:border-[#4F46E5] transition-colors">
              <span className="pixel-text text-[10px] text-white">View on Explorer</span>
              <ExternalLink className="w-4 h-4 text-[#888]" />
            </button>
          </div>
        </PixelCard>
      )}

      {/* Info Card */}
      {!verificationResult && (
        <PixelCard>
          <div className="text-center py-6">
            <Shield className="w-12 h-12 mx-auto mb-3 text-[#4F46E5]" strokeWidth={3} />
            <div className="pixel-text text-xs text-white mb-2">HOW TO VERIFY</div>
            <div className="pixel-text text-[8px] text-[#888] leading-relaxed">
              Paste the VC JSON or hash above,<br/>
              or scan a QR code to verify<br/>
              credential authenticity.
            </div>
          </div>
        </PixelCard>
      )}

      {/* Scan Modal */}
      {isScanning && (
        <ScanQrModal
          onClose={() => setIsScanning(false)}
          onScan={(data) => {
            setVcInput(data);
            setIsScanning(false);
            handleVerify();
          }}
        />
      )}
    </div>
  );
}
